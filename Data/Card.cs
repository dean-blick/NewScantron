...
namespace ScantronRevived.Data
{
    public class Card
    {
        ...
        private void Uncompress()
        {
            ...
            {
                uncompressed_string = "";
                hashtag_location = raw_card_data.IndexOfAny(compression_symbol.ToCharArray());
            ...
            for (int i = 9; i < 29; i += 5)
            {
                if (i < 14)
                {
                    for (int j = 4; j >= 0; j--)
                    {
                        ...
                        for (int k = 0; k < 5; k++)
                        {
                            answer += card_lines[i + k][j];
                        }
                        int CheckIfNoResponse = 0;
        ...
        /// <param name="c">Third bubble's darkness.</param>
        /// <returns>Which bubble is darkest. Defaults to 0.</returns>
        private int GetDarkestBubbleAnswer(int a, int b, int c, int d, int e)
        {
            int max = 0;
            max = Math.Max(a, max);
            max = Math.Max(b, max);
        ...
        public string ToSingleAnswerString(int numberofquestions)
        {
            string card_info = "";
            string answer = "";
            string version;
            ...
            for (int i = 0; i < numberofquestions; i++)
            {
                ...
                card_info += GetDarkestBubbleAnswer(response[i].Answer[0],
                    response[i].Answer[1],
                    response[i].Answer[2],
                    response[i].Answer[3],
                    response[i].Answer[4]);
